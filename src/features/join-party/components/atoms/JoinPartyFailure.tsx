...
import { useSearchParams, type ReadonlyURLSearchParams, usePathname, useRouter } from "next/navigation";
...
function $getRetryReference(initialSearchParams: ReadonlyURLSearchParams, pathname: string): string {
  ...
  if (isInt(retry$)) {
    if (retry$ === "3") {
      ...
      rootSearchParams.set("redirectReason", redirectReasons.MaximumJoinRetriesReached);
...
function JoinPartyFailure({ error }: JoinPartyFailureProps): JSX.Element | null {
