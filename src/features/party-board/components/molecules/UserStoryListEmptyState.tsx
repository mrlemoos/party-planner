...
import { type ReactNode, type JSX, isValidElement, useMemo, Fragment } from "react";
...
export default function UserStoryListEmptyState({ callToAction, className }: UserStoryListEmptyStateProps): JSX.Element {
  ...
  const isOwner = useMemo(() => isSignedIn && user?.id === ownerUserId, [ownerUserId, user?.id, isSignedIn]);
    ...
    <div className={cls("flex flex-col justify-center items-center gap-6 p-3", className)} style={{ minHeight: 400 }}>
      ...
      {isOwner ? (
        ...
        <span className={cls("text-gray-500 text-sm select-none")}>
          The party owner has not added any user stories yet. Maybe you should get a hot cup of coffee ☕️
        </span>
      )}
    </div>
  );
