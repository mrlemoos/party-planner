...
export default function UserStoryListItemOrderChevrons({
  ...
  const { rewriteStories, stories, voteSession, createVoteSession, partyId } = usePartyBoardContext();
  ...
  const handleMoveToTop = useCallback(
    function handleMoveToTop$() {
      ...
      const newStories = [...stories.sort(({ storyId: storyId$ }) => (storyId$ === storyId ? -1 : 0))];
