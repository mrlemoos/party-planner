...
export async function POST(
  _: NextRequest,
  { params: { partyId } }: JoinPartyBackendContext,
) {
  const { userId } = auth();

  if (!userId) {
    return NextResponse.json(
      handleException('JP-1', 'You must be logged in to join a party.'),
      {
        status: 401,
      },
    );
  ...
  if (!user) {
    return NextResponse.json(
      handleException('JP-2', 'Your credentials could not be found.'),
      {
        status: 401,
      },
    );
  ...
  try {
    ...
    const hasJoined = await repo.joinParty({
      userId,
      userDisplayName,
      partyId,
    });
    ...
    return NextResponse.json(
      handleException(
        'JP-3',
        'An internal error occurred that has impeded you from joining the party.',
        {
          hasJoined,
          userId,
          userDisplayName,
    ...
    return NextResponse.json(
      handleException(
        'JP-4',
        'An error occurred while trying to join the party.',
        {
          error,
          partyId,
        },
