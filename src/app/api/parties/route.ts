...
export async function POST() {
  ...
  if (!userId) {
    return NextResponse.json(
      handleException('CP-1', 'You must be logged in to create a party.'),
      {
        status: 401,
      },
    );
  ...
  if (!user) {
    return NextResponse.json(
      handleException('CP-3', 'Could not load the user data.'),
      {
        status: 401,
      },
    );
  ...
  const ownerDisplayName = [user?.firstName, user?.lastName]
    .filter(Boolean)
    .join(' ')
    .trim();
