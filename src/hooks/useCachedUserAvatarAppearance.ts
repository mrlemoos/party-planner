...
type UserAvatarAppearanceState = ExtractAtomValue<
  typeof UserAvatarAppearanceAtom
>[string];
...
interface GetUserAvatarAppearance {
  (
    userId: string,
    options: GetUserAvatarAppearanceOptions,
  ): UserAvatarAppearanceState;
}
...
export default function useCachedUserAvatarAppearance() {
  const [avatarAppearanceByUserId, cacheAvatarAppearance] = useAtom(
    UserAvatarAppearanceAtom,
  );
  ...
  const getUserAvatarAppearance = useCallback<GetUserAvatarAppearance>(
    (userId, { orDefaultsTo }) => {
      ...
      const defaultAvatarConfiguration =
        orDefaultsTo === 'random' ? $getRandomAvatarColorMatch() : orDefaultsTo;
