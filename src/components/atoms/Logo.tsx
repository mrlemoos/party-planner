...
function Logo({ className, isInteractive = false, isMinimum = false, ...props }: LogoProps): JSX.Element {
  return (
    <span
      className={cls(
        "text-sm rounded-full  py-2 cursor-pointer",
        isMinimum ? "w-8 h-8 flex items-center justify-center bg-purple-100 text-lg" : "px-3 py-2 bg-purple-800 text-white relative",
        "transition-all",
      ...
      {...props}
    >
      {!isMinimum && <span className={cls("absolute text-2xl", isMinimum ? "-right-5 bottom-0" : "-top-3 -right-3", "-order-1")}>🎉</span>}
