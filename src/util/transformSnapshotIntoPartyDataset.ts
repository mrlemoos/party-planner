 ...
 * @see {@link Party}
 */
function transformSnapshotIntoPartyDataset(
  ...
  party.stories = party.stories
    ? (convertFirebaseObjectToArray(party.stories) as Story[])
    : [];
  party.members = party.members
    ? (convertFirebaseObjectToArray(party.members) as PartyMember[])
    : [];
